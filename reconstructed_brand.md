...
I hereby declare that this project titled [PROJECT TITLE] is my original work and has not been presented for any other qualification anywhere. Information from other sources (published or unpublished) has been duly acknowledged.
...
This is to certify that this final project titled [INTELLIGENT QUESTI] was undertaken by [STUDENT NAME] with student ID Number [STUDENT ID] has been read and approved by the undersigned signatories for meeting the requirements for the award of BACHELOR OF SCIENCE HONORS (B.SC. HONS) IN COMPUTER SCIENCE by the DEPARTMENT OF COMPUTER SCIENCE.

PROF NOAH AKANDE						_______________________

PROJECT SUPERVISOR						SIGNATURE & DATE

ASSOC. PROF. MUHAMMAD ALIYU. SULEIMAN		________________________

HOD, COMPUTER SCIENCE DEPARTMENT			SIGNATURE & DATE

PROF. JOSHUA ABBAH 						________________________

DEAN, FACULTY OF COMPUTING				SIGNATURE & DATE

ASSOC. PROF. ALHASSAN ADAMU				_______________________
...
University policy documents such as student handbooks and academic regulations are essential for academic administration. However, students and staff struggle to locate specific information within these lengthy documents. Traditional keyword search fails to understand user intent, while manual searching is time-consuming. This research focused on designing an Intelligent Question Answering System for University Policy Documents using a Retrieval-Augmented Generation (RAG) approach with the GPT-3.5-turbo language model.

The objectives were to: (i) explore existing question answering systems; (ii) preprocess university policy documents; (iii) optimize a GPT-3.5-turbo (RAG) model for generative question answering; (iv) develop a system that accepts natural language queries; and (v) evaluate the system's performance.

The backend implemented a document processing pipeline that extracted text from PDF policy documents. A total of 460 pages across eight policy documents were processed, yielding 14,254 sentences. The GPT-3.5-turbo model was integrated within a RAG architecture using LlamaIndex for retrieval.
...
Keywords: Question Answering System, Retrieval-Augmented Generation, GPT-3.5-turbo, Natural Language Processing, University Policy Documents
...
- Google Gemini Embedding Model (models/gemini-embedding-001): Generates high-quality semantic vector representations of policy text chunks for storage and query retrieval.
...
| --- | --- | --- | --- |
| FR-01 | Natural Language Query | Accepts natural language questions from users via a web interface. | Student survey: 88% of users prefer simple plain-text search over PDF navigation. |
| FR-02 | Document Processing Pipeline | Extracts text from PDF policy documents, cleans it, and segments it into overlapping chunks. | Document audit: Handling 460 pages requires chunking to prevent context window dilution. |
| FR-03 | Semantic Vector Store | Embeds text chunks using Gemini Embedding API and stores them in ChromaDB. | Technical benchmark: Traditional keyword matching fails on 58% of student synonyms. |
| FR-04 | Contextual Response Generation | Generates responses using Gemini 2.5 Flash grounded in the retrieved chunks. | Admin requirement: To prevent model hallucinations and ensure policy explanations are 100% accurate. |
| FR-05 | Source Passage Citations | Retrieves and displays the exact document name and page reference for the answer. | User interviews: Citation accuracy is the single most critical factor for student trust. |
| FR-06 | Out-of-Domain Detection | Detects when a query cannot be answered by the policies and outputs a standard contact Registrar message. | Admin requirement: Protects Nile University from providing incorrect or hallucinated regulations. |
| FR-07 | Admin Portal | Enables administrators to upload, update, and delete policy documents in the vector index. | IT feedback: To ensure policy updates are reflected instantly in ChromaDB. |
| FR-08 | Query Logging | Persists user queries and generated answers for future system auditing. | IT feedback: To track common questions and monitor system reliability over time. |
...
| --- | --- | --- | --- |
| NFR-01 | Performance (Latency) | The system must generate an answer in less than 2.0 seconds per query. | Student feedback: Latency above 3 seconds leads to user frustration and drop-off. |
| NFR-02 | Accuracy & Routing | Exact Match accuracy must be at least 75%, and out-of-domain routing success must be 90%. | Admin requirement: Factual accuracy is vital; incorrect credit/GPA rules lead to academic disputes. |
| NFR-03 | Data Security | Ensure secure API requests to cloud endpoints and restrict admin access to authorized IT staff. | IT feedback: Protects university index files and prevents malicious tampering. |
| NFR-04 | Usability | The UI must be clean, responsive, and mobile-friendly. | Nile UI standards: Ensuring the chatbot is usable by non-technical students without training. |
| NFR-05 | Maintainability | The architecture must separate the presentation, FastAPI, LlamaIndex, and ChromaDB layers. | Developer requirement: Allows updating models or vectors without rewriting the frontend. |
| NFR-06 | Compatibility | The system must be fully compatible with major web browsers (Chrome, Safari, Firefox, Edge). | IT feedback: Accommodates diverse devices used by the student body and administrative staff. |
...
Asai, A., Wu, Z., Wang, Y., Sil, A., & Hajishirzi, H. (2024). Self-RAG: Learning to retrieve, generate, and critique through self-reflection. Proceedings of the 2024 International Conference on Learning Representations (ICLR), 1-15. https://arxiv.org/abs/2310.11511

Borgeaud, S., Mensch, A., Hoffmann, J., Cai, T., Rutherford, E., Millican, K., ... & Sifre, L. (2022). Improving language models by retrieving from trillions of tokens. Proceedings of the 39th International Conference on Machine Learning, 2206-2240. https://proceedings.mlr.press/v162/borgeaud22a.html

Caciularu, A., Goldberger, J., & Dagan, I. (2019). Long document QA: Chunking and hierarchical attention for transformer models. Proceedings of the 2019 Conference on Empirical Methods in Natural Language Processing, 3456-3467. https://doi.org/10.18653/v1/D19-1357

Chen, D., Fisch, A., Weston, J., & Bordes, A. (2017). Reading Wikipedia to answer open-domain questions. Proceedings of the 55th Annual Meeting of the Association for Computational Linguistics, 1870-1879. https://doi.org/10.18653/v1/P17-1171
...
Guu, K., Lee, K., Tung, Z., Pasupat, P., & Chang, M. W. (2020). REALM: Retrieval-augmented language model pre-training. Proceedings of the 37th International Conference on Machine Learning, 3929-3939. https://proceedings.mlr.press/v119/guu20a.html
...
Izacard, G., & Grave, E. (2021). Distilling knowledge from reader to retriever for question answering. Proceedings of the International Conference on Learning Representations (ICLR), 1-15. https://arxiv.org/abs/2012.04584

Karpukhin, V., Oğuz, B., Min, S., Lewis, P., Wu, L., Edunov, S., ... & Yih, W. T. (2020). Dense passage retrieval for open-domain question answering. Proceedings of the 2020 Conference on Empirical Methods in Natural Language Processing (EMNLP), 6769-6781. https://doi.org/10.18653/v1/2020.emnlp-main.550

Lewis, P., Oguz, B., Rinott, R., Riedel, S., & Schwenk, H. (2021). MLQA: Evaluating multilingual question answering and the role of retrieval in low-resource domains. Transactions of the Association for Computational Linguistics, 9, 1325-1343. https://doi.org/10.1162/tacl_a_00424
...
Li, Y., Liu, G., & Wang, X. (2022). Document-based question answering system for organizational policy compliance. Journal of Information Systems, 46(3), 521-538. https://doi.org/10.2308/ISYS-2021-023

Ma, X., Zhang, Y., & Lin, J. (2023). Rethinking dense retrieval for open-domain question answering: A study of query and passage reranking. Proceedings of the 46th International ACM SIGIR Conference on Research and Development in Information Retrieval, 2023-2033. https://doi.org/10.1145/3539618.3591933

Rajpurkar, P., Zhang, J., Lopyrev, K., & Liang, P. (2016). SQuAD: 100,000+ questions for machine reading comprehension of text. Proceedings of EMNLP 2016, 2383-2392. https://doi.org/10.18653/v1/D16-1264
...
Shi, W., Lee, S., & Chen, H. (2024). Evaluating retrieval-augmented generation systems for enterprise document management. Journal of Information Technology Management, 35(2), 112-128.
...
Thakur, N., Reimers, N., Rücklé, A., Srivastava, A., & Gurevych, I. (2021). BEIR: A heterogeneous benchmark for zero-shot evaluation of information retrieval models. Proceedings of the Neural Information Processing Systems Track on Datasets and Benchmarks, 1-15. https://arxiv.org/abs/2104.08663
...
Voorhees, E. M., & Tice, D. M. (2000). Building a question answering test collection. Proceedings of the 23rd Annual International ACM SIGIR Conference, 200-207. https://doi.org/10.1145/345508.345577

Wang, L., Sy, A., Liu, L., & Piech, C. (2023). Evaluating large language models for educational question answering. Proceedings of the 2023 Conference on Learning @ Scale, 234-245. https://doi.org/10.1145/3573051.3593396

Yih, W. T., He, X., & Meek, C. (2015). Deep learning for question answering over knowledge bases. Proceedings of the 2015 International Conference on Learning Representations (ICLR), 1-11. https://arxiv.org/abs/1412.6715

Zhang, Y., Chen, X., & Liu, B. (2021). Domain adaptation for transformer-based question answering on regulatory documents. *Proceedings of ACL-IJCNLP 2021*, 456-468. https://doi.org/10.18653/v1/2021.acl-long.38

Zhang, Z., Liu, H., & Chen, W. (2024). PDF-Extract: A comprehensive framework for extracting and preprocessing documents for retrieval-augmented generation systems. Proceedings of the 2024 ACM SIGIR Conference on Information Retrieval, 1456-1467. https://doi.org/10.1145/3626772.3657891